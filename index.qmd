...
log_res_plot %>% # excluding extremely large CIs
  ...
  scale_x_log10(breaks = c(0.1, 0.5, 1, 2, 3, 6, 9, 12), minor_breaks = NULL, limits = c(0.1,6)) +
  #xlim(c(0.1, 6))+
  theme(
...
summary(glm(formula = dx ~ age + dx_hpv + iud + iud_years + smokes_packs_year, 
            family = "binomial", data = df))

#Same regression, with exponentiated parameters
parsnip::fit(logistic_reg(), dx ~ age + dx_hpv + iud + iud_years + smokes_packs_year, data = df)
...
# prep_dataset <- df %>% 
#   dplyr::select(-c(dx_cin:biopsy)) %>% #drop unused target variables
...
# 
# ## TESTING
# obj <- pROC::roc(nnet[[3]]$pred$obs, nnet[[3]]$pred$Yes)
# 
# ggroc(obj) +
#   ggtitle(paste0('ROC Curve ', '(AUC = ', obj$auc, ')')) +
#   geom_abline(intercept = 1, color="gray")
# 
# #sens, spec and threshold
# blah <- tibble(sens = obj$sensitivities,
#                spec = obj$specificities,
#                tres = obj$thresholds)
# 
# #manually calculate AUC
# AUC =sum(blah$spec [1:length(blah$tres)]*diff(c(0, 1 - blah$sens [1:length(blah$tres)])))
# 
# ggplot(blah, aes(y = sens, x = 1 - spec)) + 
#   geom_line() +
#   geom_abline(intercept = 0, color="gray") +
#   geom_text(aes(x=0.5, y = 0.33), label=paste0("AUC: ", scales::number(AUC, accuracy = 0.001)))
# 
# 
# 
# 
...
# 
# 
# get_auc <- function(dataset, outcome_var, model) {
#   dataset <- get(dataset)
#   model <- get(model)
#   roc(dataset[[outcome_var]], predict(model, validation, type="prob")$Yes)$auc %>% as.numeric
# }
# 
# roc(nnet[[1]]$pred$obs, nnet[[1]]$pred$Yes)
# 
# 1:7 %>% 
#   map(roc(nnet[[.x]]$pred$obs, nnet[[.x]]$pred$Yes))
# 
# lapply(1:7, function(x){roc(naive_bayes[[x]]$pred$obs, naive_bayes[[x]]$pred$Yes)$auc}) 
